 ...
 *
 * FIXES:
 * 1. `totalDone` was hardcoded to 24 — now reads from the task store
 * 2. Projects count was hardcoded to 3 — now reads from the project store
 */
...
function SettingsRow({
  ...
  return (
    ...
    >
      ...
      <Text style={{ flex: 1, fontSize: 14, fontWeight: '500', color: isRed ? '#EF4444' : t.textPrimary }}>
        {label}
      </Text>
...
export default function Profile() {
  ...
  // FIX: derive real stats from stores instead of hardcoded values
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: t.bg }}>
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        <Text style={{ fontSize: 24, fontWeight: '700', color: t.textPrimary, letterSpacing: -0.8, marginBottom: 16 }}>
          Profile
        </Text>
        ...
        <View style={[styles.card, { backgroundColor: t.surface, borderColor: t.border, flexDirection: 'row', alignItems: 'center', gap: 14 }]}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 18, fontWeight: '700', color: t.textPrimary, letterSpacing: -0.3 }}>
              {user?.name}
            </Text>
        ...
        {/* Stats — FIX: real values from stores */}
        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 20 }}>
          {[
            ...
            <View key={s.label} style={[styles.statCard, { backgroundColor: t.surface, borderColor: t.border }]}>
              ...
              <Text style={{ fontSize: 20, fontWeight: '700', color: t.textPrimary, letterSpacing: -0.5, textAlign: 'center' }}>
                {s.num}
              </Text>
              <Text style={{ fontSize: 10, color: t.textTertiary, textTransform: 'uppercase', letterSpacing: 0.4, textAlign: 'center', marginTop: 2 }}>
                {s.label}
              </Text>
            </View>
          ))}
        ...
        <View style={[styles.card, { backgroundColor: t.surface, borderColor: t.border }]}>
          ...
          <SettingsRow icon="🎨" iconBg={t.accentLight} label="Accent Color"
            ...
            showChevron onPress={() => router.push('/profile/appearance')} />
        </View>
