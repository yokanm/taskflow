...
export default function TaskDetail() {
  ...
  const handleToggle = async () => {
    ...
    setToggling(true);
    const next = { ...task, status: task.status === 'DONE' ? 'TODO' : ('DONE' as typeof task.status) };
    setTask(next);
    try {
  ...
  const dueDate = task.dueDate ? new Date(task.dueDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : null;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: t.bg }}>
      ...
      <View style={[styles.hero, { backgroundColor: t.accent }]}>
        ...
        {task.project && <Text style={styles.heroProject}>{task.project.name}</Text>}
        <Text style={styles.heroTitle} numberOfLines={2}>{task.title}</Text>
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 12 }}>
          <View style={styles.heroBadge}><Text style={styles.heroBadgeText}>🔵 {task.status.replace('_', ' ')}</Text></View>
          <View style={styles.heroBadge}>
            <Text style={styles.heroBadgeText}>
              {task.priority === 'HIGH' ? '🔴' : task.priority === 'MEDIUM' ? '🟡' : '🟢'} {task.priority.charAt(0) + task.priority.slice(1).toLowerCase()}
            </Text>
          </View>
        </View>
      </View>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 20, paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
          {dueDate && (
            <View style={[styles.infoCard, { flex: 1, backgroundColor: isOverdue ? '#FEF2F2' : t.surface2,
              borderColor: isOverdue ? '#FECACA' : t.border }]}>
              <Text style={{ fontSize: 10, fontWeight: '600', color: t.textTertiary, textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 4 }}>📅 Deadline</Text>
              <Text style={{ fontSize: 13, fontWeight: '600', color: t.textPrimary, marginBottom: 6 }}>{dueDate}</Text>
              <View style={[styles.badge, { backgroundColor: isOverdue ? '#FEE2E2' : t.accentLight, borderColor: isOverdue ? '#EF4444' : t.accent }]}>
          ...
          <View style={[styles.infoCard, { flex: 1, backgroundColor: t.surface2, borderColor: t.border }]}>
            <Text style={{ fontSize: 10, fontWeight: '600', color: t.textTertiary, textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 4 }}>🎯 Priority</Text>
        ...
        {task.description && (
          <View style={{ marginBottom: 20 }}>
            ...
            <Text style={{ fontSize: 13, color: t.textSecondary, lineHeight: 21 }}>{task.description}</Text>
          </View>
        )}
        ...
        <View style={{ marginBottom: 20 }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            ...
            {task.subTasks.length > 0 && <Text style={{ fontSize: 11, fontWeight: '600', color: t.accent }}>{subDone} of {task.subTasks.length}</Text>}
          </View>

          {task.subTasks.length > 0 && (
            <>
              <View style={[styles.subProgress, { backgroundColor: t.surface2 }]}>
                <View style={[styles.subProgressFill, { width: `${task.subTasks.length > 0 ? (subDone / task.subTasks.length) * 100 : 0}%`, backgroundColor: t.accent }]} />
              </View>
              {task.subTasks.map((s) => (
                <TouchableOpacity key={s.id} onPress={() => handleToggleSubtask(s.id)}
                  style={[styles.subtaskRow, { borderBottomColor: t.border }]}>
                  <View style={[styles.cb, { borderColor: s.done ? t.accent : t.border, backgroundColor: s.done ? t.accent : 'transparent' }]}>
                    {s.done && <View style={styles.check} />}
                  </View>
                  <Text style={{ fontSize: 13, color: s.done ? t.textTertiary : t.textPrimary, textDecorationLine: s.done ? 'line-through' : 'none', flex: 1 }}>{s.title}</Text>
                </TouchableOpacity>
              ))}
            </>
          )}
          ...
          <View style={[styles.addSubtask, { borderColor: t.border }]}>
            <TextInput value={subtaskInput} onChangeText={setSubtaskInput} placeholder="Add a subtask…"
              placeholderTextColor={t.textTertiary} style={{ flex: 1, fontSize: 12, color: t.textPrimary }}
              onSubmitEditing={handleAddSubtask} />
            <TouchableOpacity onPress={handleAddSubtask}><Text style={{ color: t.accent, fontSize: 18 }}>+</Text></TouchableOpacity>
          </View>
        </View>
      ...
      <View style={[styles.footer, { backgroundColor: t.surface, borderTopColor: t.border }]}>
        <Button label={isDone ? 'Mark as Todo' : 'Mark as Done'} style={{ flex: 1 }}
          onPress={handleToggle} loading={toggling} />
        <Button label="Edit Task" variant="secondary" style={{ flex: 1 }}
